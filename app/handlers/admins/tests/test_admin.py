...
def test_detail_keyboard(test) -> InlineKeyboardMarkup:
    ...
    if test.is_active:
        builder.button(
            text="🔴 Faolsizlashtirish", callback_data=f"ta_toggle:{test.id}"
        )
    else:
    ...
    builder.button(
        text="🗑 Sessiyalarni tozalash", callback_data=f"ta_clear_sessions:{test.id}"
    )
...
@router.callback_query(
    F.data.startswith("ta_schedule_custom:"), IsAdmin(admin_ids=ADMINS)
)
...
@router.callback_query(
    F.data.startswith("ta_schedule_clear:"), IsAdmin(admin_ids=ADMINS)
)
...
@router.callback_query(
    F.data.startswith("ta_clear_sessions:"), IsAdmin(admin_ids=ADMINS)
)
