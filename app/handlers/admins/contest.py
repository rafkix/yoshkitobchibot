...
async def cn_pick_confirm(callback: CallbackQuery):
    ...
    if not winner:
        await callback.message.edit_text(
            ...
            f"Min referal cheklovi: {contest.min_referrals} ta",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
...
async def cn_create_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "(masalan: <i>Iyun oyi referal poygasi</i>)",
        parse_mode="HTML",
...
async def cn_create_description(message: Message, state: FSMContext):
    ...
    await message.answer(
        "3️⃣ Konkurs tugmasi bosilganda chiqadigan matnni kiriting:\n"
        "Matn shart bo‘lmasa, pastdagi tugmani bosing.",
        reply_markup=skip_cancel_reply_keyboard(),
    )
...
async def cn_create_prize(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "▶️ Konkursni boshlash uchun quyidagi tugmani bosing.",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
...
async def cn_edit_score_user(message: Message, state: FSMContext):
    ...
    if not user:
        return await message.answer("❌ Bunday ID dagi foydalanuvchi topilmadi.")
